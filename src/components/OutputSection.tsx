  ...
  const outputFormats: OutputFormat[] = [
    {
      ...
      formatter: (commands) => {
        const regexArray = commands.map(cmd => `"${cmd.regex}"`).join(',\n    ');
        return `// VSCode settings.json format
{
  ...
  "search.regexPatterns": [
    ...
    },
    {
      ...
      formatter: (commands) => {
        const patterns = commands.map(cmd => `  - pattern: "${cmd.regex}"\n    description: "${cmd.originalCommand}"`).join('\n');
...
const patterns = [
${commands.map(cmd => `  /${cmd.regex}/g, // ${cmd.originalCommand}`).join('\n')}
];`;
      }
    },
    {
